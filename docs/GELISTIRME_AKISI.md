...
|---|-------|--------|-------|--------------|
| A | **Takip modu** | Sliding window → hedef kilitlenince sadece pencere takibi (1-9 inference/kare) | 🔄 | Host: kilitliyken 20+ FPS, kayıpta tam taramaya dönüş |
| B | **UART offset çıkışı** | GPIO4/5 UART2, `T+035Y-012\n` formatı, 115200 baud | ⏳ | Serial monitörde format doğru |
| C | **Kayıp/kilitleme mantığı** | Güven eşiği, ardışık kayıp sayacı, yeniden kilitleme | ⏳ | 3 kare kayıp → tam tarama |
| D | **ESP-DSP hızlandırma** | Xtensa SIMD ile conv2d dot product (3-5x) | ⏳ | Inference süresi ölçümü |
